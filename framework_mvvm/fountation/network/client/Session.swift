...
public class Session {
    ...
    init(delegate: SessionDelegateType) {
    ...
    public func send<T: Request>(
        ...
        completionHandler completion: ((Result<T.Response, DXError>) -> Void)? = nil) -> SessionTask?
    {
        ...
        completionHandler completion: ((Result<T.Response, DXError>) -> Void)? = nil) -> SessionTask?
    {
        ...
        do {
            ...
            callbackQueue.execute { completion?(.failure(DXError.unknownError)) }
            return nil
        }
        ...
        sessionTask.onResult.delegate(on: self) { (self, value) in
            switch value {
            case (_, _, let error?):
                print(error) //TODO derek
                let error = DXError.responseFailed // LineSDKError.responseFailed(reason: .URLSessionError(error))
            ...
            default:
                let error = DXError.responseFailed // LineSDKError.responseFailed(reason: .nonHTTPURLResponse)
                callbackQueue.execute { completion?(.failure(error)) }
            }
        }
        ...
        done: @escaping ((HandleResult<T.Response>) throws -> Void)) throws
    {
        ...
        case .terminator(let terminator):
            do {
                ...
                throw DXError.dataParsingFailed // LineSDKError.responseFailed(reason: .dataParsingFailed(T.Response.self, data, error))
            }
        }
    }
}
